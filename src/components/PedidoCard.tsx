import { FaCheckCircle, FaMotorcycle, FaClock, FaTimes, FaHamburger, FaGlassWhiskey, FaCoffee, FaBeer, FaUtensils } from "react-icons/fa";
...
export default function PedidoCard({ pedido, status, now, onStatusChange, onOpenDetails }: PedidoCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`bg-white/5 backdrop-blur rounded-xl border ${colors.border} overflow-hidden hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 mb-3`}
    >
                ...
                return (
                  <div key={idx} className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      <span className={`w-6 h-6 rounded-full ${colors.bg} ${colors.text} flex items-center justify-center`}>{itemIcon(itemObj.nome, itemObj.icon)}</span>
