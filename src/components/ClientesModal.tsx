    ...
    <AnimatePresence>
      {open && (
        <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <motion.div className="relative w-full max-w-2xl max-h-[80vh] overflow-y-auto ds-modal" initial={{ y: 24, scale: 0.97 }} animate={{ y: 0, scale: 1 }} exit={{ y: 24, scale: 0.97 }}>
            <div className="px-5 py-4 border-b theme-border flex items-center justify-between">
              ...
              <button className="p-2 rounded border theme-border hover:bg-white/5 text-zinc-300" aria-label="Fechar" onClick={onClose}><FaTimes /></button>
            </div>
            <div className="p-4">
              {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {items.map(c => (
                    <button key={c.uuid} className="text-left p-3 rounded border theme-border theme-surface hover:bg-white/5 text-zinc-200 transition" onClick={()=> onSelect({ id: c.uuid, nick: c.nick, estrelas: c.estrelas, gasto: c.gasto, simpatia: c.simpatia, compras: c.compras })}>
